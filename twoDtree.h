...
private:
   ...
   */
   ...
   /**
    * copyHelper for the copy method
    * @param subRoot
    * @return
    */
    ...
    * @param image
    */
   void render(const Node* subRoot, PNG & image);

   /**
    * helper for prune
    * @param subRoot
    */
   void prune(Node* subRoot, double pct, int tol);

    /**
     * prune distance calculator
     * @param root
     * @param leaf
     */
   int pruneDistance(Node* baseRoot, Node* leaf);

   int countLeaves(Node *subRoot);
   int numberOfLeavesWithinTolerance(Node *baseRoot, Node *subRoot, int tol);
